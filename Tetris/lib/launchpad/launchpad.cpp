...
LaunchpadBase::LaunchpadBase() 
{
    ...
    sendmessage_mutex = CreateMutex(NULL, NULL, L"launchpadmsg");
}
...
void LaunchpadBase::sendMessage(unsigned int first_byte, ...) {
    ...
    WaitForSingleObject(sendmessage_mutex, INFINITE);    // set mutex to prevent wrong bytes sequence
    ...
    ReleaseMutex(sendmessage_mutex);
}
...
void LaunchpadPro::displayText(unsigned int color, bool loop, unsigned int speed,
                               std::string text) {
	if (isConnected() == false) return;
    std::vector<BYTE> message;
...
void LaunchpadPro::playMidiFile_speed(std::string file, int speed) {
    MidiFile midifile;
    MidiEvent *mev;
    ...
    midifile.read(file);
    midifile.joinTracks();
    int BPM = midifile.getTicksPerQuarterNote() * speed;
